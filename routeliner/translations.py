...
TRANSLATIONS = {
    ...
    'Журнал ещё не заведён.':
        'The log has not been set up yet.',
    'Руководство (PDF)':
        'Manual (PDF)',
    'Журнал':
        'Log',
    ...
    'ООО «Информ++»':
        'Inform++ LLC',
    'страница для предприятий':
        'page for enterprises',
    'Журнал: {p}':
        'Log: {p}',
    'Исходный код':
        'Source code',
    ...
    'Чертёж профиля':
        'Profile drawing',
    'Инструменты Routeliner':
        'Routeliner tools',
    'О модуле…':
        'About',
    'Версия, ссылки, история изменений, руководство и журнал':
        'Version, links, version history, manual and log',
    'Журнал работы':
        'Work log',
    ...
    'Параметры те же, что у инструмента {src}, но результат динамический. Слой в памяти пересчитывается сам при правке геометрии маршрутов, таблицы событий или ведомости, в том числе при несохранённой правке и при изменении файла CSV или Excel на диске. Рядом создаётся таблица ошибок.\n\nПривязка хранится в проекте и восстанавливается при его открытии. Чтобы отключить пересчёт, удалите слой событий из проекта.\n\nПоля слоя совпадают с полями результата инструмента {src}.':
        'The parameters are those of tool {src}, but the result is live. The in-memory layer is recalculated by itself when the route geometry, the event table or the ledger changes, including unsaved edits and a CSV or Excel file changed on disk. An error table is created next to it.\n\nThe binding is stored in the project and restored when the project opens. To stop the recalculation, remove the event layer from the project.\n\nThe fields of the layer are those of the result of tool {src}.',
